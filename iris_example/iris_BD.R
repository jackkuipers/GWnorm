...
if (method_val == "BD_bd") {
  n_its <- round(n_its*2/3)
  BDgraph_out <- BDgraph::bdgraph(data, iter = n_its, burnin = n_its/4, algorithm = "bdmcmc", g.prior = 0.5, df.prior = b)
} else {
  BDgraph_out <- BDgraph::bdgraph(data, iter = n_its, burnin = n_its/4, algorithm = "rjmcmc", g.prior = 0.5, df.prior = b)
}
